...
class UR3Controller:
    ...
    def execute_trajectory(self, trajectory, z_height=None,
        ...
        路径点来源优先级：
            1. trajectory.generated_points（3D模式含层间过渡）
            2. trajectory.generate_path_points() + z_height（从2D线段采样）
        """
        # 获取路径点：优先使用预生成的3D点，否则从segments采样
        if trajectory.generated_points is not None:
            raw_points = np.array(trajectory.generated_points, dtype=float)
            if raw_points.ndim == 2 and raw_points.shape[1] >= 3:
                # 已经是3D点 [x, y, z]
                path_3d = raw_points[:, :3]
            else:
                # 预生成的是2D点，需要补Z
                if z_height is None:
                    z_height = float(self.get_tcp()[2])
                path_3d = np.column_stack([raw_points,
                                           np.full(len(raw_points), z_height)])
            print(f"使用预生成路径点，共 {len(path_3d)} 个")
        else:
        ...
        has_pregen_3d = (trajectory.generated_points is not None
                         and np.array(trajectory.generated_points).ndim == 2
                         and np.array(trajectory.generated_points).shape[1] >= 3)
        print(f"工作高度 Z: 自动（3D路径中使用自带Z坐标）"
              if has_pregen_3d
              else f"工作高度 Z: {z_height:.2f} mm")
        print(f"路径点数: {len(path_3d)}，总长度: {trajectory.get_total_length():.1f} mm (仅2D段)")
...
if __name__ == "__main__":
    ...
    controller.move_to_point([100,100,50])
